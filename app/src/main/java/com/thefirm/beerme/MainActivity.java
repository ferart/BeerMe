...
import com.thefirm.beerme.ble.ble.BleDevicesScanner;
import com.thefirm.beerme.ble.ble.BleManager;
import com.thefirm.beerme.ble.ble.BleUtils;
import com.thefirm.beerme.ble.uart.UartCommunication;
import com.thefirm.beerme.ble.uart.UartInterface;
import com.thefirm.beerme.ble.utils.DialogUtils;
import com.thefirm.beerme.ble.utils.ExpandableHeightExpandableListView;
import com.thefirm.beerme.servertalk.ServerCommunicationService;
import com.thefirm.beerme.servertalk.SimpleCallback;
...
public class MainActivity extends AppCompatActivity implements BleManager.BleManagerListener, BleUtils.ResetBluetoothAdapterListener {
    ...
    private UartCommunication uartCommunication;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        serverCommunicationService=ServerCommunicationService.getInstance();
        serverCommunicationService.startScheduler();

        serverCommunicationService.setSimpleCallback(commandCallback);
    ...
    protected void onDestroy() {
        serverCommunicationService.shutdownSchedulerService();
...
//            Log.d(TAG, "record -> lenght: " + length + " type:" + type + " data" + data);
    ...
    public void onConnected() {
        ...
        uartCommunication=new UartCommunication(MainActivity.this);
        if (uartCommunication!=null){

        }
        //todo start bt scheduler
    }
    ...
    private int aux=0;
    SimpleCallback commandCallback= new SimpleCallback() {
        @Override
        public void success(final BeerPojo beerPojo) { //time to send info to the BT
            aux++;
            if (aux==4) {
                serverCommunicationService.ackSent(beerPojo.getOrderId());
                aux=0;
            }
            /*
            if (beerPojo!=null && uartCommunication!= null && mBleManager.getConnectedDevice()!=null && serverCommunicationService!=null) {
               uartCommunication.sendDataToDevice("1",null);
               aux++;
                if (aux==6) {
                    serverCommunicationService.ackSent(beerPojo.getOrderId());
                    aux=0;
                ...
                /*        new UartInterface.SendDataCompletionHandler() {
                    @Override
                    public void sendDataResponse(String data) {
                        Log.v(TAG,"Info has been sent to the device");
                        serverCommunicationService.ackSent(beerPojo.getOrderId());
                    }
